...
[`../schema.sql`](../schema.sql). This directory tries to break it, nine ways, and
...
Six are refused outright with an error. Three — 4, 5 and 8 — are *allowed to run* and
must have no effect, which is the more interesting failure mode: a forged write that is
